...
function CoreWeb_entry_meta() {
	if ( 'post' === get_post_type() ) {
		...
		printf( '<span class="byline"><span class="author vcard">%1$s<span class="screen-reader-text">%2$s </span> <a class="url fn n" href="%3$s">%4$s</a></span></span>',
			...
			_x( 'Author', 'Used before post author name.', 'CoreWeb' ),
	...
	if ( current_theme_supports( 'post-formats', $format ) ) {
		printf( '<span class="entry-format">%1$s<a href="%2$s">%3$s</a></span>',
			sprintf( '<span class="screen-reader-text">%s </span>', _x( 'Format', 'Used before post format.', 'CoreWeb' ) ),
...
function CoreWeb_entry_date() {
	...
	printf( '<span class="posted-on"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
		_x( 'Posted on', 'Used before publish date.', 'CoreWeb' ),
...
if ( ! function_exists( 'CoreWeb_entry_taxonomies' ) ) :
/**
...
function CoreWeb_entry_taxonomies() {
	...
	if ( $categories_list && CoreWeb_categorized_blog() ) {
		printf( '<span class="cat-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
			_x( 'Categories', 'Used before category names.', 'CoreWeb' ),
			$categories_list
		);
	}
...
if ( ! function_exists( 'CoreWeb_post_thumbnail' ) ) :
/**
...
if ( ! function_exists( 'CoreWeb_excerpt' ) ) :
	 ...
	 */
	function CoreWeb_excerpt( $class = 'entry-summary' ) {
...
if ( ! function_exists( 'CoreWeb_excerpt_more' ) && ! is_admin() ) :
/**
 ...
 */
function CoreWeb_excerpt_more() {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		/* translators: %s: Name of current post */
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'CoreWeb' ), get_the_title( get_the_ID() ) )
	);
	return ' &hellip; ' . $link;
}
...
function CoreWeb_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'CoreWeb_categories' ) ) ) {
		...
		set_transient( 'CoreWeb_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so CoreWeb_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so CoreWeb_categorized_blog should return false.
		return false;
	}
}
endif;
...
function CoreWeb_category_transient_flusher() {
	...
	// Like, beat it. Dig?
	delete_transient( 'CoreWeb_categories' );
}
add_action( 'edit_category', 'CoreWeb_category_transient_flusher' );
add_action( 'save_post',     'CoreWeb_category_transient_flusher' );

if ( ! function_exists( 'CoreWeb_the_custom_logo' ) ) :
/**
